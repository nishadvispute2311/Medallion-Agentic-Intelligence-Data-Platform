...
#     row = (
#         spark.table("agentic_ai_catalog.guardian_schema.guardian_agent_control")
...
#     row = (
#         spark.table("agentic_ai_catalog.drift_schema.drift_control_table")
#              .filter(f"table_name = '{table_name}'")
#              .orderBy(expr("updated_at DESC"))
#              .limit(1)
#              .collect()
...
@dlt.table(
    name="silver_refunds",
    comment="Silver refunds with Guardian + Drift enforcement"
)
def silver_refunds():

    TABLE_NAME = "bronze_refunds"

    bronze_df = dlt.read("bronze_refunds")
...
@dlt.table(
    name="quarantine_refunds",
    comment="refunds blocked by Guardian or Drift"
)
def quarantine_refunds():
